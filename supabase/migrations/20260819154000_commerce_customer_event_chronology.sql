-- Customer tag chronology and marketing-consent chronology are independent.
-- Shopify's authoritative consent timestamp can be old while a subscription
-- tag update is new. Use the dedicated consent timestamp only for consent and
-- preserve the webhook trigger/update time for subscription projection order.
...
declare
  ...
  v_consent_observed_at timestamptz := coalesce(
    nullif(p_signal->>'marketing_consent_observed_at', '')::timestamptz,
    (p_signal->>'occurred_at')::timestamptz
  );
...
begin
  ...
  if v_email is not null and v_consent <> 'unknown' then
    insert into public.commerce_customer_state (
      ...
      v_shop, v_email, v_customer_id, v_consent, v_consent_observed_at
    )
