-- A later paid order may reopen a refund/cancellation hard exit only when the
-- durable Shopify consent projection is currently subscribed and that grant
-- was observed no later than the purchase. Consent is intentionally stripped
-- before the legacy projection runs, so this re-entry decision belongs in the
