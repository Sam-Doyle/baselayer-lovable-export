...
| [site-architecture.md](wiki/site-architecture.md) | technical | 2026-08-18 | Full React/Vite/Netlify architecture, Shopify Storefront checkout, Sanity CMS, Puppeteer prerender, security/caching, native build-time Judge.me pipeline with corrected verification/product filtering, analytics event queue and checkout measurement boundaries, GA4 reserved-parameter collision (`source` overwrote session attribution site-wide), `_ga`-not-`_gl` as the subdomain cross-domain test, Meta catalog content IDs as bare variant IDs, Brevo hybrid lifecycle ownership |
| [technical-seo.md](wiki/technical-seo.md) | technical | 2026-08-18 | **New 2026-08-18.** Crawl/index/schema/prerender delivery mechanics. The governing rule that every `<head>` value in this codebase has two writers (prerender plugin + React component) and its three confirmed instances: duplicate Product schema on the three landing pages, title/description drift on 10 of 14 static routes, and the rAF-polling failure that shipped the homepage as a 2,959-byte skeleton. Plus GSC warning triage (match the schema `name`, not the URL; "page with redirect" is the www variant), sitemap `lastmod` semantics and the shallow-clone hazard, and live curl verification snippets. **Rev 2 (2026-08-18):** Instance 4 — the five advertorials, `/lp` and `/product/*` are rewritten to a generic shell whose raw HTML carries the homepage `<title>` and no canonical, so social unfurlers and AI crawlers read every paid landing page as the homepage. Plus the crawl-health baseline: 60/60 sitemap URLs clean, with five 2-hop `/blog/*` redirect chains and one internal link to a 301 as the only residual hygiene, all on zero-traffic pages |
| [performance-metrics.md](wiki/performance-metrics.md) | technical | 2026-08-18 | Build compression/code-splitting, responsive media, preload/skeleton LCP work, CLS remediation and web-vitals monitoring, Judge.me 320px photo delivery, the intermittent prerender read-stream race, and the LCP-deferral/prerender collision |
...
| [seo-strategy.md](wiki/seo-strategy.md) | marketing | 2026-08-18 | Technical SEO setup (SEO.tsx: meta tags, OG, canonical, JSON-LD schemas — FAQPage re-enabled, supersedes prior disabled status), sitemap generation, robots.txt, prerender strategy, Netlify caching/security headers, content SEO, keyword strategy, internal linking architecture, performance optimization priorities, SEO baseline audit (zero impressions, no sitemap submitted, GA4 snapshot), tech-debt audit (soft-404 + Offer schema gaps across 5 files), backlink outreach targets, site-wide meta-injection bug fix (homepage meta shipped on every page). **Rev 5 (2026-08-18):** no page-2 quick wins exist — position 4–20 is empty at every threshold, best position anywhere is 26, 366 impressions and 0 clicks over 90 days. The finding is cannibalization: `/articles/best-moisturizer-for-men` and `/comparisons/best-mens-face-moisturizers-compared` collide on 21 queries carrying a third of all impressions, with a recommended split and the advertorial indexability decision reframed as prerender-first |
...
| [conversion-learnings.md](wiki/conversion-learnings.md) | conversion | 2026-08-18 | ROAS and traffic routing, measured tier economics, single-bottle discount floor, subscription/F&F guardrails, cold-traffic and advertorial frameworks, conservative competitor disclosure, native review histogram/photo-strip behavior and its open low-volume gate test, the above-fold aggregate that made a 5-review sample salient, and the unmeasured 15%-off skin-concern quiz |
| [shipping-economics.md](wiki/shipping-economics.md) | technical | 2026-08-18 | rev 4 — canonical **88 g product-plus-carton weight per unit** (96 g single / 184 g 2-pack including mailer), with quoted carrier rates still in the same 4 oz / 8 oz tiers ($7.12 single / $7.75 2-pack landed), three counterintuitive rules (DIM irrelevant under 1 cu ft, GA Cubic is a trap for light parcels, below-Commercial label source is worth ~$1.86/order after the 2026-07-12 ounce-tier elimination), supplier and Shopify configuration, airless-pump protection analysis in the purchased 9x12 poly mailer, and the product-scoped selling plan that applied a fixed $35 policy to the $68 2-pack |
