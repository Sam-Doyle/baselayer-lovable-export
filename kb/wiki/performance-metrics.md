...
revision: 1
sources: [vite.config.ts, netlify.toml, index.html, src/App.tsx, src/pages/Index.tsx, src/components/HeroSection.tsx, package.json]
codePaths:
  - vite.config.ts
  - netlify.toml
  - index.html
...
- **No runtime performance monitoring.** No `web-vitals` library or CrUX reporting baked into the client. Lighthouse/PageSpeed must be run manually.
...
No Lighthouse scores or Core Web Vitals baselines are documented in the codebase or `docs/` directory. The only performance-related doc is `docs/comparison-content-briefs.md` (content, not technical).

**Recommendation:** Run Lighthouse CI or PageSpeed Insights on the 5 key pages (`/`, `/face-cream`, `/articles`, `/ingredients`, `/skin-concerns`) and capture baseline scores here for regression tracking.
